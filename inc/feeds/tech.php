<?php 
	...
	if(!$loggedin&&!isset($_GET['t'])){
		...
		if(isset($_POST['newpost'])&&!empty($_POST['newpost'])){
			...
			if(strlen($newpost)>0){
				if(mysqli_query($sqli, "INSERT INTO tech_feed (posterid, text, date) VALUES ('".$userid."','".$newpost."','".date("Y-m-d H:i:s")."')")){
	...
	if(isset($_GET['t'])&&!empty($_GET['t'])){
		...
		if(isset($_POST['newreply'])&&!empty($_POST['newreply'])){
			...
			if(strlen($newreply)>0){
				if(mysqli_query($sqli, "INSERT INTO tech_comments (posterid, opid, text, date) VALUES ('".$userid."', '".$threadid."','".$newreply."','".date("Y-m-d H:i:s")."')")){
		...
		$thread = mysqli_query($sqli, "SELECT text FROM tech_feed WHERE id='".$threadid."'");
		if(mysqli_num_rows($thread)==1){
			$thread = mysqli_fetch_array($thread)[0];
			$threadopid = mysqli_query($sqli, "SELECT posterid FROM tech_feed WHERE id='".$threadid."'");
			...
			$date = mysqli_query($sqli, "SELECT date FROM tech_feed WHERE id='".$threadid."'");
			...
			$comments = mysqli_query($sqli, "SELECT * FROM tech_comments WHERE opid='".$threadid."' ORDER BY id DESC");
...
<?php
		...
		$feed = mysqli_query($sqli, "SELECT * FROM tech_feed ORDER BY id DESC LIMIT 0 , 10");
